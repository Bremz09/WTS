...
if Calc == "Female Team Sprint":
    ...
    # --- Track Animation (Side View) ---
    st.header("Track Animation")
    ...
    y_riders = [0.18, 0.0, -0.18]   # stagger slightly so labels don't overlap
    max_dist_anim = 760

    # Static traces: track line + 10m marker segments + marker labels
    # These are NEVER included in frames so they never redraw
    marker_xs, marker_ys = [], []
    ...
    # Initial rider traces at t=0 (trace indices 3, 4, 5)
    init_rider_traces = []
    ...
    # Build animation frames — only update the 3 rider traces, NO layout/axis changes
    anim_frames = []
    for i, t in enumerate(anim_times):
        ...
        anim_frames.append(go.Frame(
            data=frame_data,
            traces=[3, 4, 5],   # only touch the 3 rider dots — static traces untouched
            name=f"f{i}"
        ))
